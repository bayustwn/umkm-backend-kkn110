...
export const registerUMKM = async (c: Context) => {
  try {
    ...
    return c.json({ message: error instanceof Error ? error.message : "Gagal mendaftakan UMKM" }, 500);
  }
}
...
export const getOtherUmkm = async (c: Context) => {
  try {
    ...
    const umkm = await prisma.$queryRaw<{ id: string; name: string; address: string; phone: string; description: string; status: string; image: string; latitude: number; longitude: number; category: string; jumlahProduk: bigint }[]>`
      SELECT 
        u.id, u.name, u.address, u.phone, u.description, u.status, u.image, u.latitude, u.longitude,
        c.name AS category,
    ...
    return c.json({ message: "Gagal mendapatkan preview UMKM" }, 500);
  }
};
...
export const getAllUmkm = async (c: Context) => {
  try {
    ...
    if (result.length > 0) {
      ...
      return c.json({ message: "Tidak ada data UMKM" }, 404);
    }
  } catch (error) {
    return c.json({ message: "Gagal mendapatkan data UMKM" }, 500);
  }
};
...
export const deleteUMKM = async (c: Context) => {
  try {
    ...
    return c.json({ message: error instanceof Error ? error.message : "Gagal menghapus UMKM" }, 500);
  }
}
...
export const approveUMKM = async (c: Context) => {
  try {
    ...
    return c.json({ message: error instanceof Error ? error.message : "Gagal menyetujui UMKM" }, 500);
  }
}
...
export const getAllCategory = async (c: Context) => {
  try {
    ...
    if (categories.length > 0) {
      ...
      return c.json({ message: "Tidak ada kategori" }, 404);
    }
  } catch (error) {
    return c.json({ message: "Gagal mendapatkan kategori" }, 500);
  }
};
...
export const addCategory = async (c: Context) => {
  try {
    ...
    return c.json({ message: error instanceof Error ? error.message : "Gagal menambahkan kategori" }, 500);
  }
};
...
export const deleteCategory = async (c: Context) => {
  try {
    ...
    return c.json({ message: error instanceof Error ? error.message : "Gagal menghapus kategori" }, 500);
  }
};
...
export const getDetailUmkm = async (c: Context) => {
  ...
  try {
    ...
    if (umkm) {
      ...
      return c.json({ message: "Tidak ada produk untuk UMKM ini" }, 404);
    }
  } catch (error) {
    return c.json({ message: "Gagal mendapatkan produk" }, 500);
  }
};
...
export const uploadProduct = async (c: Context) => {
  try {
    ...
    return c.json({ message: error instanceof Error ? error.message : 'Gagal upload produk' }, 500);
  }
}
...
export const updateUMKM = async (c: Context) => {
  try {
    ...
    return c.json({ message: error instanceof Error ? error.message : "Gagal memperbarui UMKM" }, 500);
  }
}
...
export const updateProduct = async (c: Context) => {
  try {
    ...
    return c.json({ message: error instanceof Error ? error.message : "Gagal memperbarui produk" }, 500);
  }
}
